...
def process_mzml_file(file_input, path_output):
    ...
    # Read mzML file
    try:
        ms_file = load_mzml_data(str(file_input))
    ...
    features_mapped = features_mapped[abs(features_mapped["mz_offset"]) < 0.5]
    ...
    if len(features_mapped) == 0:
        open(path_output/"no_features", "wt").close()
        return 0
    ...
    ms2_selected, ms2_unmapped = extract_ms2_spectra(ms_file, features_mapped)
...
def extract_ms2_spectra(ms_file, features_mapped: pd.DataFrame):
    ...
    def clean_ms2(ms2_info):
        ...
        for i, (spec, precursor_mz, charge) in enumerate(zip(spec_all, ms2_info["precursor_mz"], ms2_info["charge"])):
            spec_clean = clean_spectrum(
                spec, max_mz=precursor_mz-1.5/abs(charge), noise_threshold=0.01, ms2_da=0.05)
    ...
    return clean_ms2(selected_ms2), clean_ms2(unmapped_ms2)
