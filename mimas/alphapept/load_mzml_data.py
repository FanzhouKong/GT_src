...
def load_mzml_data(file: str, n_most_abundant=100) -> tuple:
    ...
    ms_run["mass_list_ms1"] = (mass_list_ms1)
    ms_run["int_list_ms1"] = (int_list_ms1)
    ...
    # return ms_run
    ms_run["indices_ms2"] = _index_ragged_list(ms_run["mass_list_ms2"])
    if len(ms_run["mass_list_ms2"]) > 1:
        ms_run["mass_list_ms2"] = np.concatenate(ms_run["mass_list_ms2"])
    else:
        ms_run["mass_list_ms2"] = np.array(ms_run["mass_list_ms2"])
    if len(ms_run["int_list_ms2"]) > 1:
        ms_run["int_list_ms2"] = np.concatenate(ms_run["int_list_ms2"])
    else:
        ms_run["int_list_ms2"] = np.array(ms_run["int_list_ms2"])
    return ms_run
...
def scan_time_in_minutes(self):
    ...
    if self._scan_time_in_minutes is None:
        ...
        if self._scan_time_unit.lower() == "second":
            ...
            self._scan_time_in_minutes = self._scan_time * 60.0
        elif self._scan_time_unit.lower() == "unicorns":
            # This is an unit for the Fiehn Lab's private format.
            self._scan_time_in_minutes = self._scan_time / 60.0
        else:
...
def _extract_mzml_info(input_dict: dict) -> tuple:
    ...
    """
    rt = scan_time_in_minutes(input_dict)
