...
@performance.compile_function(compilation_mode="numba")
...
@performance.performance_function(compilation_mode="numba-multithread")
...
def filter_hills(hill_data:np.ndarray, hill_ptrs:np.ndarray, int_data:np.ndarray, hill_check_large:int =40, window:int = 1, hill_peak_factor:float = 2, large_peak:int = 40) -> (np.ndarray, np.ndarray):
    """Filters large hills.
    ...
    check_large_hills(range(len(large_peaks)), large_peaks, hill_ptrs, hill_data, int_data, to_remove, large_peak, hill_peak_factor, window)
...
def report_(idx:np.ndarray, isotope_charges:list, isotope_patterns:list, iso_idx:np.ndarray, stats:np.ndarray, sortindex_:np.ndarray, hill_ptrs:np.ndarray, hill_data:np.ndarray, int_data:np.ndarray, rt_:np.ndarray, rt_idx:np.ndarray, results:np.ndarray, lookup_idx:np.ndarray):
    ...
    for i, k in enumerate(pattern):
        ...
        if int_.sum() > most_intense_pattern:
            ...
            rt_start_idx = rt_idx[idx_][0]
            rt_end_idx = rt_idx[idx_][-1]
            rt_apex_idx_real = rt_start_idx + int_.argmax()
            # feature_sn = ms1_int_max_apex / (int_[0]+int_[-1]) * 2
    ...
    results[idx,:] = np.array([mz, mz_std, mz_most_abundant, charge, rt_start, rt_apex, rt_end, fwhm, n_isotopes, mass, ms1_int_sum_apex, ms1_int_sum_area, ms1_int_max_apex, ms1_int_max_area, rt_start_idx, rt_apex_idx_real, rt_end_idx])
...
def feature_finder_report(query_data:dict, isotope_patterns:list, isotope_charges:list, iso_idx:np.ndarray, stats:np.ndarray, sortindex_:np.ndarray, hill_ptrs:np.ndarray, hill_data:np.ndarray)->pd.DataFrame:
    ...
    results = np.zeros((len(isotope_charges), 17))
    ...
    df = pd.DataFrame(results, columns = ['mz','mz_std','mz_most_abundant','charge','rt_start','rt_apex','rt_end','fwhm','n_isotopes','mass', 'ms1_int_sum_apex', 'ms1_int_sum_area', 'ms1_int_max_apex', 'ms1_int_max_area', 'rt_start_idx','rt_apex_idx','rt_end_idx'])
...
def map_ms2(feature_table:pd.DataFrame, query_data:dict, map_mz_range:float = 1, map_rt_range:float = 0.5, map_mob_range:float = 0.3, map_n_neighbors:int=5, search_unidentified:bool = False, **kwargs)->pd.DataFrame:
    ...
    for neighbor in range(map_n_neighbors):
        ...
        if 'select_windows_ms2' in query_data.keys():
            select_windows_ms2 = query_data['select_windows_ms2']
            if np.min(select_windows_ms2[:,1]) > 0:
                mass_check = np.bitwise_and(ref_df['mz_matched'].values >= select_windows_ms2[:,0], ref_df['mz_matched'].values <= select_windows_ms2[:,1])
            else:
                mass_check = np.abs(ref_df['mz_offset'].values) <= map_mz_range
        else:
            mass_check = np.abs(ref_df['mz_offset'].values) <= map_mz_range
